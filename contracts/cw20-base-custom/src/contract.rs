...
use cosmwasm_std::{
    to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdError, StdResult, Uint128, Addr, Order,
};
...
use crate::state::{
    ...
    TOKEN_INFO, LOCK_OPERATOR, LOCK_ADDR_INFO, LockAddressInfo, LockAddressResponse, LockAddressMsg,
};
...
pub fn instantiate(
    ...
    LOCK_OPERATOR.save(deps.storage, &info.sender)?;
...
pub fn execute(
    ...
    //check locked address
    check_lock_address(deps.as_ref(), &env, &info.sender)?;
    ...
    match msg {
        ...
        ExecuteMsg::Lock { msgs } => execute_lock(deps, env, info, msgs),
        ExecuteMsg::Unlock { msgs } => execute_unlock(deps, env, info, msgs),
        ExecuteMsg::UpdateOperator { new_operator } => execute_update_operator(deps, env, info, new_operator),
    }
}
...
pub fn execute_transfer(
    ...
    //check lock address
    check_lock_address(deps.as_ref(), &env, &rcpt_addr)?;
...
pub fn execute_update_minter(
    ...
    //check lock address
    if let Some(minter) = new_minter.clone() {
        let minter_addr = deps.api.addr_validate(&minter)?;
        check_lock_address(deps.as_ref(), &env, &minter_addr)?;
    }
...
pub fn execute_lock(
    ...
    env: Env,
    info: MessageInfo,
    locks: Vec<LockAddressMsg>,
) -> Result<Response, ContractError> {

    update_lock_address(deps, &env, &info, locks, true)?;
...
pub fn execute_unlock(
    ...
    env: Env,
    info: MessageInfo,
    unlocks: Vec<LockAddressMsg>,
) -> Result<Response, ContractError> {

    update_lock_address(deps, &env, &info, unlocks, false)?;
...
pub fn execute_update_operator(
    ...
    let lock_operator = LOCK_OPERATOR.load(deps.storage)?;
    if lock_operator != info.sender{
        return Err(ContractError::Unauthorized {});
    }

    let new_operator_addr = deps.api.addr_validate(&new_operator)?;
    check_lock_address(deps.as_ref(), &env, &new_operator_addr)?;
    
    LOCK_OPERATOR.save(deps.storage, &new_operator_addr)?;
...
pub fn update_lock_address(
    deps: DepsMut,
    env: &Env,
    info: &MessageInfo,
    msgs: Vec<LockAddressMsg>,
    add: bool,
    ...
    for lock in msgs.iter() {

        let lock_addr = deps.api.addr_validate(&lock.address)?;
        if lock_operator == lock_addr {
            return Err(ContractError::LockWrongAddress {});
        }

        if let Some(expire) = lock.expire {
            if expire <= env.block.time.seconds() {
                return Err(ContractError::LockExpired {});
            }
        }

        if add {
            let balance = query_balance(deps.as_ref(), lock_addr.to_string())?.balance;

            LOCK_ADDR_INFO.update(
                deps.storage, 
                &lock_addr, 
                |v| -> StdResult<_> {
                    match v {
                        Some(mut lnfo) => {
                            lnfo.balance = balance;
                            lnfo.expire = lock.expire;
                            Ok(lnfo)
                        },
                        None => {
                            let lock_time = env.block.time.seconds();
                            let expire = lock.expire;
                            Ok(LockAddressInfo { 
                                lock_time,
                                balance,
                                expire
            ...
            LOCK_ADDR_INFO.remove(deps.storage, &lock_addr);
        }
    }
...
pub fn check_lock_address(
    deps: Deps,
    env: &Env,
    lock_addr: &Addr,
) -> Result<(), ContractError> {

    let lock_result = LOCK_ADDR_INFO.load(deps.storage, lock_addr);
    if let Ok(lock) = lock_result {
        if lock.is_expired(env.block.time) == false {
            return Err(ContractError::LockedAddress {});
        }
    }
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::LockAddressList {
            include_expired, 
            start_after,
            limit,
        } => to_binary(&query_lock_addresses(deps, env, include_expired, start_after, limit)?),
        QueryMsg::LockAddressInfo { address } => to_binary(&query_lock_info(deps, address)?),
    }
}
...
pub fn query_lock_info(
    deps: Deps, 
    address: String,
) -> StdResult<LockAddressInfo> {

    let lock_addr = deps.api.addr_validate(&address)?;
    let lock_info = LOCK_ADDR_INFO.load(deps.storage, &lock_addr)?;
    
    Ok(
        LockAddressInfo { 
            lock_time: lock_info.lock_time,
            balance: lock_info.balance,
            expire: lock_info.expire,
        }
    )
}

pub fn query_lock_addresses(
    deps: Deps, 
    env: Env,
    include_expired: bool,
    start_after: Option<String>,
    limit: Option<u32>,
) -> StdResult<LockAddressResponse> {
    let limit = limit.unwrap_or(10).min(100) as usize;
    let start_addr = maybe_addr(deps.api, start_after)?;
    let start = start_addr.as_ref().map(Bound::exclusive);
    
    let lock_address: Vec<Addr> = LOCK_ADDR_INFO
        .range(deps.storage, start, None, Order::Ascending)
        .filter(|r| {
            include_expired || !r.as_ref().unwrap().1.is_expired(env.block.time)
        })
        .take(limit)
        .map(|lock| {
            lock.unwrap().0
    ...
    let operator = LOCK_OPERATOR.load(deps.storage)?;
...
pub fn migrate(deps: DepsMut, env: Env, msg: MigrateMsg) -> Result<Response, ContractError> {
    ...
    let operator_addr = deps.api.addr_validate(&msg.lock_operator)?;
    check_lock_address(deps.as_ref(), &env, &operator_addr)?;

    LOCK_OPERATOR.save(deps.storage, &operator_addr)?;
        ...
        fn test_migrate() {
            ...
            app.execute(
                ...
                CosmosMsg::Wasm(WasmMsg::Migrate {
                    contract_addr: cw20_addr.to_string(),
                    new_code_id: cw20_id,
                    msg: to_binary(&MigrateMsg { lock_operator: "lock_operator".to_string() }).unwrap(),
                }),
            )
            .unwrap();
